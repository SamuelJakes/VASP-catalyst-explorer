...
#Added by Samuel Jakes to make reading the graph easier
graphWidth = 0
...
#   Draw their labels
# Modified by Samuel Jakes to make label shifting more intuitive (relative to y scale)
# Constant factors experimentally determined coeficients to make the label shift look good.
        offset = self.ax.get_ylim()
        offset = offset[1]*0.01*(graphWidth/1.8868)
        for key in self.statesList.keys():
            state = self.statesList[key]
            self.ax.text(state.leftPointx + state.labelOffset[0], state.leftPointy + state.labelOffset[1]*offset + offset,
                state.label,
                color=state.labelColor,
                verticalalignment='bottom')
            self.ax.text(state.leftPointx  + state.textOffset[0], state.leftPointy + state.textOffset[1]*offset - offset,
                "" + str(state.energy),
                color=state.labelColor,
        ...
        # This multiplicative factor has been added by Samuel Jakes
        # Smaller factors make the graph wider - this can be used to make graphs fit better.
        x_range = (xlim[1] - xlim[0])/graphWidth
            ...
            if (stateBlock):
                ...
                if (line.strip()[0] == "}"):
                    ...
                    if (len(raw) != 2 and raw[0].upper().strip() != "LABEL"):
                        ...
                        #Added by Samuel Jakes to make moving labels up and down for conflict avoidance much easier
                        elif (raw[0] == "MOVEUP" or raw[0] == "MOVE UP" or raw[0] == "MOVE-UP"):
                            statesList[-1].labelOffset = (0, 6)
                            statesList[-1].textOffset = (0, 7)
                        elif (raw[0] == "MOVEDOWN" or raw[0] == "MOVE DOWN" or raw[0] == "MOVE-DOWN"):
                            statesList[-1].labelOffset = (0, -8)
                            statesList[-1].textOffset = (0, -7)
                ...
                if (len(raw) != 2):
                    ...
                    elif (raw[0] == "GRAPHWIDTH"):
                        try:
                            global graphWidth
                            graphWidth = float(raw[1])
                        except ValueError:
                            print("ERROR: Could not read integer for diagram graphwidth on line " + str(lc)+ ":\n\t"+line)
